...
import { DurationChartProps, DurationChartData } from "./types";
...
function prepareData(
  startData: DurationChartData[],
  stays: DurationChartProps["confirmedStays"]
) {
  function incArrayValue(
    arr: DurationChartData[],
    field: DurationChartData["duration"]
  ) {
    return arr.map((obj) =>
  ...
  const data = stays
    .reduce((arr, cur) => {
      ...
      if ([4, 5].includes(num as number))
        return incArrayValue(arr, "4-5 nights");
      if ([6, 7].includes(num as number))
